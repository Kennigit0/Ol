import asyncio, time, re, random, io, os, subprocess, unicodedata, json
import numpy as np
import cv2
...
# Which account this process runs as, chosen via env var:
#   OL_ACCOUNT=epic python3 ordinal_botpixel_multi.py
ACCOUNT = os.environ.get("OL_ACCOUNT", "epic")
...
ACCOUNTS = {
    ...
    "spike": {
        "api_id":   33562154,                 # ← fill in from my.telegram.org
        "api_hash": "7bea846c20a8c4c10d48b8ab26610d51",
        "phone":    "+917859846782",
    },
    "kenpachi": {
        "api_id":   0,                 # ← fill in from my.telegram.org
        "api_hash": "PASTE_KENPACHI_API_HASH",
        "phone":    "+91XXXXXXXXXX",
    },
}
...
if ACCOUNT not in ACCOUNTS:
    raise SystemExit(
        f"Unknown OL_ACCOUNT='{ACCOUNT}'. Valid options: {', '.join(ACCOUNTS)}"
    )

_cfg     = ACCOUNTS[ACCOUNT]
API_ID   = _cfg["api_id"]
API_HASH = _cfg["api_hash"]
PHONE    = _cfg["phone"]

if not API_ID or "PASTE_" in API_HASH or "XXXXXXXXXX" in PHONE:
    raise SystemExit(
        f"Account '{ACCOUNT}' is missing real api_id/api_hash/phone — "
        f"edit the ACCOUNTS dict at the top of this file."
    )
...
client           = TelegramClient(f"ordinal{ACCOUNT}_session", API_ID, API_HASH)
last_action_time = 0
...
LOG_FILE_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), f"botpixel_log_{ACCOUNT}.txt")
...
def extract_line_symbol(line):
    ...
    true ZWJ-compound emoji stays whole, while a separate, merely-adjacent
    repeat of a simple symbol (e.g. "♠♠", two independent spade characters
    used for visual emphasis) collapses to the single "♠" used elsewhere.
    """
        ...
        if image_bytes:
            ...
            if lib_count is not None:
                ...
                if candidates is None:
                    log("[MONSTER GROUP] No-AI method failed — will pause for manual solve")
...
CONTROL_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), f"control_{ACCOUNT}.txt")

def show_notification():
    """Shows a persistent notification with Pause/Resume/Stop buttons"""
    try:
        _notif_id = str(100 + list(ACCOUNTS.keys()).index(ACCOUNT))
        subprocess.run([
            "termux-notification",
            "--id", _notif_id,
            "--title", f"Ordinal Legacy Bot [{ACCOUNT}]",
